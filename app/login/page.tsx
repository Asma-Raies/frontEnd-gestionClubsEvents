...
export default function LoginPage() {
  ...
  const getRoleFromPayload = (payload: any): string => {
    ...
    return normalized; // ADMIN, MODERATEUR, ETUDIANT
  };
  ...
  const redirectBasedOnRole = (role: string) => {
    if (role === "ADMIN") {
      ...
      router.push("/dashboard/moderateur");
    } else {
      router.push("/dashboard"); // Étudiant ou rôle par défaut
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Construction de l'utilisateur final
      const user = {
